...
"""  
    __valid_models = ("VVX 301", "VVX 311", "VVX 401", "VVX 411", "VVX 150", "VVX 250", "VVX 350", "VVX 450")
    __valid_actions = ("DND",)

    def __validate_basics(self):
        Method validates self for (1)base profile type for 'Generic' or not, (2)model against internal tuple, __valid_models
        and (3)firmware version against internal tuple, __valid_versions.
        INPUTS: None
        OUTPUT: Returns result as boolean.
        if self.baseprofile != "Generic":
            logging.error("Invalid base profile type for operation. Supports 'Generic' mode only")
            return False
        else:
            if self.model not in self.__valid_models:
                logging.error(f"Invalid model type, accepts {self.__valid_models}")
                return False
            else:
                if not self.__swVer:
                    logging.error(f"Invalid firmware version. Supported versions are {self.__valid_versions}")
                    return False 
        return True


    def __validate_actions(self, action):
        Method validates action against internal tuple, __valid_actions.
        INPUTS: action as string
        OUTPUT: Returns result as boolean.
        if action not in self.__valid_actions:
            return False
        else:
            return True


    def simulateKeyEventCombo(self, action, swType="ucs", ctype="application/json"):
        
        Method calls internal httpRequest to POST "simulateKeyEvent" : "/api/v1/mgmt/simulateKeyEvent".
        INPUTS: *model as string (supported values found in __valid_models),
                *action as string (supported values found in __valid_actions), 
                *swType as string (supported values are "ucs", ...),  
                ctype as string(Content-Type), 
        OUTPUT: Returns result as boolean.
        

        if not self.__validate_basics():
            return False        

        if not self.__validate_actions(action):
            logging.error(f"Invalid action type, accepts {self.__valid_actions}")
            return False
        
        # Dict of commands by software, ver, model, and actions
        commands_list = { "ucs" : { 
                                    "6.1.0" : { 
                                              "VVX 301" : { "DND" : [{"Home":.5},{"ArrowDown":.5},{"ArrowRight":.5},{"Select":.5}] },
                                              "VVX 311" : { "DND" : [{"Home":.5},{"ArrowDown":.5},{"ArrowRight":.5},{"Select":.5}] },
                                              "VVX 401" : {},
                                              "VVX 411" : {},
                                              "VVX 150" : { "DND" : [{"Home":.5},{"ArrowRight":.5},{"ArrowRight":.5},{"ArrowRight":.5},{"Select":.5}] }, 
                                              "VVX 250" : { "DND" : [{"Home":.5},{"Dialpad5":.5}] }, 
                                              "VVX 350" : { "DND" : [{"Home":.5},{"Dialpad5":.5}] },
                                              "VVX 450" : { "DND" : [{"Home":.5},{"Dialpad4":.5}] }
                                              }  
                                  }                 
                        }

        body_dict = {}
        c = commands_list[swType][self.__swVer][self.model][action]
        for i in range( len(c) ):
            for k,v in c[i].items():
                body_dict = { 'data' : { 'Type': 'Tap', 'KeyName': k } }
                logging.debug(f"Body passed: {body_dict}")
                self.__httpRequest(qpath="simulateKeyEvent", rtype="POST", ctype=ctype, rdata=body_dict)
                time.sleep(v)
                body_dict.clear()
